...
async function prepareAssets() {
  ...
  const dir = path.join(os.tmpdir(), "iqc-assets");
  ...
  bgPath = path.join(dir, "Iqcbyrin.png");
  emojiJsonPath = path.join(dir, "emoji-apple.json");

  await download(BG_URL, bgPath);
  await download(APPLE_EMOJI_JSON_URL, emojiJsonPath);
...
async function renderImage(text) {
  ...
  const textTotalHeight =
    textLines.length * s.fontSize + (textLines.length - 1) * lineGap;
  ...
  if (timestampOnNewRow) {
    ...
    const lastLineTop =
      currentBubbleY +
      paddingTop +
      (textLines.length - 1) * (s.fontSize + lineGap);
